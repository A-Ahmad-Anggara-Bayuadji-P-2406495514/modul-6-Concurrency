...
Dalam pengerjaan Milestone 4, dilakukan simulasi slow request untuk memahami keterbatasan server yang bersifat single-threaded. Melalui penambahan endpoint /sleep, server diperintahkan untuk berhenti sejenak selama 10 detik menggunakan thread::sleep sebelum memberikan respon. Karena server ini hanya memiliki satu thread utama untuk menangani semua koneksi secara berurutan dalam sebuah loop, permintaan /sleep tersebut akan menahan seluruh proses eksekusi thread. Akibatnya, jika ada permintaan lain yang masuk secara bersamaan (misalnya akses ke path /), permintaan tersebut akan tertahan di antrean dan tidak akan diproses sampai durasi tidur 10 detik selesai dan respon pertama dikirimkan. Fenomena ini menunjukkan bahwa server single-threaded tidak efisien dalam menangani banyak pengguna secara simultan karena satu proses yang memakan waktu lama dapat memblokir akses bagi pengguna lainnya.
